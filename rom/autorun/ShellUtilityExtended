--source: http://www.computercraft.info/forums2/index.php?/topic/24815-	
...
--<td><img src="/i/t.gif" class="i_p0" title="Public paste, everybody can see this paste." alt="" /> <a href="/DW3LCC3L">Monitor Mirror v2.1</a></td>
...
for _,name in pairs(tPastebinUserNames) do
    ...
    if http.checkURL(site) then
        ...
        if data then
            ...
            for i,k in string.gmatch (data, '<td><img src="/i/t.gif" class="i_p0" title="Public paste, everybody can see this paste." alt="" /> <a href="/(%w+)">(.-)</a></td>') do
...
if turtle then
    ...
    local tEquip = {'left', 'right'}
    local tDirection = {'left', 'right', 'forward', 'back', 'down', 'up'}
    local tTurn = {'left', 'right'}
    local tSlots = {}
    for i=1,16 do
        tSlots[i]=tostring(i)
    end
    ...
    local function completeGo( shell, nIndex, sText )
        if nIndex == 1 then
            return completeMultipleChoice(sText,tDirection)
        end
    end
    --Turn -- List sides
    local function completeTurn( shell, nIndex, sText )
        if nIndex == 1 then
            return completeMultipleChoice(sText,tTurn)
        end
    end
    -- Equip -- List slots and sides.
    local function completeEquip( shell, nIndex, sText )
        if nIndex == 1 then
            return completeMultipleChoice(sText,tSlots,true)
        elseif nIndex == 2 then
            return completeMultipleChoice(sText,tEquip)
        end
    end
    -- Unequip -- List sides.
